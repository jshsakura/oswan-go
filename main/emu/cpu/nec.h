...
#define IncWordReg(Reg) 					\
	uint32_t tmp = (uint32_t)I.regs.w[Reg]; \
	uint32_t tmp1 = tmp+1;					\
...
#define SHL_uint8_t(c) dst <<= c;	SetCFB(dst); SetSZPF_Byte(dst);	PutbackRMByte(ModRM,(uint8_t)dst)
#define SHL_uint16_t(c) dst <<= c;	SetCFW(dst); SetSZPF_Word(dst);	PutbackRMWord(ModRM,(uint16_t)dst)
#define SHR_uint8_t(c) dst >>= c-1; I.CarryVal = dst & 0x1; dst >>= 1; SetSZPF_Byte(dst); PutbackRMByte(ModRM,(uint8_t)dst)
#define SHR_uint16_t(c) dst >>= c-1; I.CarryVal = dst & 0x1; dst >>= 1; SetSZPF_Word(dst); PutbackRMWord(ModRM,(uint16_t)dst)
#define SHRA_uint8_t(c) dst = ((int8_t)dst) >> (c-1);	I.CarryVal = dst & 0x1;	dst = ((int8_t)((uint8_t)dst)) >> 1; SetSZPF_Byte(dst); PutbackRMByte(ModRM,(uint8_t)dst)
#define SHRA_uint16_t(c) dst = ((int16_t)dst) >> (c-1);	I.CarryVal = dst & 0x1;	dst = ((int16_t)((uint16_t)dst)) >> 1; SetSZPF_Word(dst); PutbackRMWord(ModRM,(uint16_t)dst)
...
#define ADD4S {												\
	int32_t i,v1,v2,result;										\
	int32_t count = (I.regs.b[CL]+1)/2;							\
	uint32_t di = I.regs.w[IY];								\
	uint32_t si = I.regs.w[IX];								\
